...
entity ID → temperature in °C) together with the segmentation image
rendered by Genesis.
"""
...
class ThermalCameraModel(BaseSensor):
    ...
    temp_ambient_c:
        Default ambient temperature in °C assigned to pixels with no
    ...
    nuc_sigma:
        ...
        (in °C).  Applied once at construction; represents sensor NUC
        residual errors.
    noise_sigma:
        Standard deviation of per-frame Gaussian detector noise (in °C).
    bit_depth:
        Output bit depth (8 or 14 are typical for thermal cameras).
    fog_density:
        Exponential fog attenuation coefficient (m⁻¹).  0 = no fog.
    """

    SKY_ENTITY_ID: int = -1  # sentinel value for background / sky pixels
    ...
    def step(self, sim_time: float, state: dict[str, Any]) -> dict[str, Any]:
        ...
        - ``"temperature_map"`` – ``dict[int, float]`` mapping entity ID
          to surface temperature in °C.  Missing entity IDs fall back to
          ``temp_ambient_c``.
        ...
        # Sky pixels (entity id == SKY_ENTITY_ID, typically 0 or -1)
        temp_img[seg == self.SKY_ENTITY_ID] = self.temp_sky_c

        # 2. Fog attenuation on temperature (hotter → cooler for distant objects)
        if self.fog_density > 0:
            ...
            if depth is not None:
                ...
                if depth.ndim == 3:
                    depth = depth[..., 0]
                attenuation = np.exp(-self.fog_density * np.clip(depth, 0, None))
        ...
        noise = np.random.normal(0.0, self.noise_sigma, (h, w)).astype(np.float32)
    ...
    def _quantise(self, temp_img: np.ndarray) -> np.ndarray:
        ...
        # Typical LWIR range: -20 °C to 140 °C
        t_min, t_max = -20.0, 140.0
