...
class RigidEntity(KinematicEntity):
    ...
    def _resolve_terrain_cell(
        ...
        """
        Shared setup for terrain queries: validates the entity, transforms world (x, y)
        into the terrain's local grid, and reads the four height-field corners of the
        containing cell.
        ...
        if self._solver.n_envs > 0 and env_idx is None:
            gs.raise_exception(f"{caller}() requires an explicit `env_idx` for batched scenes (n_envs > 0).")
        hf = self.terrain_hf
    ...
    def get_height_at(self, x: float, y: float, env_idx: int | None = None) -> float:
        ...
        Uses triangle-based (barycentric) interpolation matching the physics mesh diagonal
        convention: each grid cell is split along the (x0,y0)-(x1,y1) diagonal into two
        triangles, consistent with the mesh generated by
        ``genesis/utils/terrain.py:convert_heightfield_to_watertight_trimesh``.
    ...
    def get_normal_at(self, x: float, y: float, env_idx: int | None = None) -> np.ndarray:
        ...
        if tx <= ty:
            ...
            dz_dx = (h10 - h00) * v_scale / h_scale
            dz_dy = (h11 - h10) * v_scale / h_scale
